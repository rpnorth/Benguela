...
def load_combine_sadcp_scanfish_data(fileSADCP, pathScan, grid_dx = 750, grid_dz = 10):
    ...
    def sadcp_select(sadcp,ind_SADCP_section):
        ...
        sadcp_out.x_m.values = np.cumsum(np.append(np.array(0),gsw.distance(sadcp_out.lon.dropna(dim='x_m').values,  
                                                                                                   sadcp_out.lat.dropna(dim='x_m').values,p=0)))
    ...
    def sadcp_scanfish_combine(sadcp,scanfish_data,ind_SADCP_section,ind_scanfish_section,grid_dx,grid_dz):
        ...
        scanfish_gridded_section, scanfish_data_section = grid_scanfish_wrapper(scanfish_data.drop('distance').isel(time=ind_scanfish_section),
    ...
    ind_SADCP_section[1] = np.logical_and(np.logical_and(np.logical_and(np.logical_and(sadcp.lon.values > 12., sadcp.lon.values < 13.5),
                                                          ...
                                                          sadcp.time.values >= sadcp.time.sel(time=np.datetime64('2016-11-19T12:55:00'), method='nearest').values ) 
    ...
    ind_scanfish_section[1] = np.logical_and(np.logical_and(np.logical_and(np.logical_and(np.logical_and(scanfish_data[1].lon.values > 12., scanfish_data[1].lon.values < 13.5),
                                                                                              ...
                                                                                              scanfish_data[1].lat.values > -27.5), 
                                                            scanfish_data[1].time.values <= sadcp.time.sel(time=np.datetime64('2016-11-20T04:30:00'),method='nearest').values),
                                             scanfish_data[1].time.values >= sadcp.time.sel(time=np.datetime64('2016-11-19T12:55:00'), method='nearest').values)
    ...
    sadcp_outOD['LADCP_CTD_Transect1'] = sadcp_select(sadcp,np.logical_and(sadcp.time.values >= np.datetime64('2016-11-20T10:00:00'),sadcp.time.values <= np.datetime64('2016-11-21T07:00:00')))
    
    sadcp_outOD['LADCP_CTD_Transect2'] = sadcp_select(sadcp,np.logical_and(sadcp.time.values >= np.datetime64('2016-11-21T14:38:00'),sadcp.time.values <= np.datetime64('2016-11-22T15:08:00')))
    
    sadcp_outOD['LADCP_CTD_Transect3'] = sadcp_select(sadcp,np.logical_and(sadcp.time.values >= np.datetime64('2016-11-24T10:33:00'),sadcp.time.values <= np.datetime64('2016-11-24T23:30:00')))
    
    sadcp_outOD['LADCP_CTD_Transect4'] = sadcp_select(sadcp,np.logical_and(sadcp.time.values >= np.datetime64('2016-11-26T08:30:00'),sadcp.time.values <= np.datetime64('2016-11-27T00:01:00')))
    
    sadcp_outOD['LADCP_CTD_Transect5'] = sadcp_select(sadcp,np.logical_and(sadcp.time.values >= np.datetime64('2016-11-27T17:43:00'),sadcp.time.values <= np.datetime64('2016-11-28T06:45:00')))

    # problem with interpolation, just removing for now
    #sadcp_outOD['LADCP_CTD_Transect7'] = sadcp_select(sadcp,np.logical_and(sadcp.time.values >= np.datetime64('2016-11-30T21:05:00'),sadcp.time.values <= np.datetime64('2016-12-01T21:35:00')))
    
    sadcp_outOD['LADCP_CTD_Transect8'] = sadcp_select(sadcp,np.logical_and(sadcp.time.values >= np.datetime64('2016-12-02T21:57:00'),sadcp.time.values <= np.datetime64('2016-12-03T07:35:00')))
    
    sadcp_outOD['LADCP_CTD_Transect9'] = sadcp_select(sadcp,np.logical_and(sadcp.time.values >= np.datetime64('2016-12-05T13:35:00'),sadcp.time.values <= np.datetime64('2016-12-06T10:20:00')))
    
    sadcp_outOD['LADCP_CTD_Transect10'] = sadcp_select(sadcp,np.logical_and(sadcp.time.values >= np.datetime64('2016-12-07T10:07:00'),sadcp.time.values <= np.datetime64('2016-12-08T02:35:00')))
    ...
    sadcp_outOD['AcrossFilament1'] =   sadcp_select(sadcp,np.logical_and(sadcp.time.values >= np.datetime64('2016-11-29T09:30:00'),sadcp.time.values <= np.datetime64('2016-11-29T15:20:00')))
